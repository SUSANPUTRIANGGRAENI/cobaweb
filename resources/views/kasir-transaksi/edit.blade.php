        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('transaksi.update', $transaksi->id) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div>
                                <div class="max-w-xl">
                                    ...
                                    <x-select id="cabang_id" name="cabang_id" class="mt-1 block w-full" required>
                                        @foreach($cabang as $item)
                                            <option value="{{ $item->id }}" {{ $item->id == $transaksi->cabang_id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </x-select>
                                ...
                                <div class="max-w-xl mt-4">
                                    ...
                                    <x-select id="users_id" name="users_id" class="mt-1 block w-full" required>
                                        @foreach($users as $item)
                                            <option value="{{ $item->id }}" {{ $item->id == $transaksi->users_id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </x-select>
                                ...
                                <div class="max-w-xl mt-4">
                                    ...
                                    <x-text-input id="total_harga" type="number" name="total_harga" class="mt-1 block w-full" value="{{ old('total_harga', $transaksi->total_harga) }}" required />
                            ...
                            <div>
                                <div class="max-w-xl">
                                    ...
                                    <x-select id="produk_id" name="produk_id" class="mt-1 block w-full" required>
                                        @foreach($produk as $item)
                                            <option value="{{ $item->id }}" {{ $item->id == $transaksi->produk_id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </x-select>
                                ...
                                <div class="max-w-xl mt-4">
                                    ...
                                    <x-text-input id="qty" type="number" name="qty" class="mt-1 block w-full" value="{{ old('qty', $transaksi->qty) }}" required />
                                    <x-input-error class="mt-2" :messages="$errors->get('qty')" />
                                </div>
                                <div class="max-w-xl mt-4">
                                    <x-input-label for="tanggal_transaksi" value="Tanggal Transaksi" />
                                    <x-text-input id="tanggal_transaksi" type="date" name="tanggal_transaksi" class="mt-1 block w-full" value="{{ old('tanggal_transaksi', $transaksi->tanggal_transaksi) }}" required />
